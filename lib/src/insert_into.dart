...
class InsertInto
  ...
  InsertInto._({
    required this.intoTable,
    this.fields = const [],
    Iterable values = const [],
    Iterable<Condition> onDuplicateKeyUpdate = const [],
  })  : this._values = values,
  ...
  InsertInto _update({
    ...
    return new InsertInto._(
        ...
        onDuplicateKeyUpdate: onDuplicateKeyUpdate ?? _onDuplicateKeyUpdate);
  }

  String toSql() {
    var sql =
        "INSERT INTO ${intoTable.toSql()} (${this.fields.map((f) => f.toSql()).join(", ")}) ";
    var values =
        this._values.map((v) => v is Serializable ? v.toSql() : objectToSql(v));
    sql += "VALUES (${values.join(", ")})";
    if (_onDuplicateKeyUpdate.isNotEmpty) {
      sql +=
          " ON DUPLICATE KEY UPDATE ${_onDuplicateKeyUpdate.map((c) => c.toSql()).join(", ")}";
    }
    return sql;
  }

  InsertOnDuplicateSetMoreStep setField(
      TableField field, TableField anotherField) {
    var condition = new EqualFieldCondition(field, anotherField);
    return _update(
        onDuplicateKeyUpdate: []
          ..addAll(this._onDuplicateKeyUpdate)
          ..add(condition));
  }

  InsertOnDuplicateSetMoreStep setObj(TableField field, dynamic object) {
